...
**Criterio de sucesso:** Implementação do botão de visualização da senha na tela de login/cadastro, correção do bug de sincronização da foto da capa da galeria no backend e refatoração do grid de visualização para exibir exatamente 5 fotos por linha no desktop de forma totalmente responsiva.
...
**Resumo para commit:** Feat: adiciona visualizacao de senha, corrige sincronizacao de capa e atualiza grid de fotos para 5 colunas no desktop.
