...
- **Nova Regra de Desconto Progressivo por Aula Individual**: Reformulada a lógica de precificação para aplicar o desconto de forma progressiva e individual para cada hora de aula. A 1ª hora mantém-se a valor cheio (ex: R$ 100,00); a 2ª hora recebe o percentual de desconto da 2ª aula (ex: 10%); e a 3ª hora recebe o percentual de desconto da 3ª aula (ex: 20%). Os descontos são salvos individualmente no banco de dados para manter a integridade fiscal de cada hora agendada.
- **Simplificação Explanatória no Modal**: A mensagem padrão do modal de desconto foi simplificada para 'Agende até 3 aulas no mesmo dia e ganhe descontos progressivos na 2ª e 3ª aula!'. O detalhamento de tarifas foi reestruturado em lista sem marcadores com bullets claros, mostrando os preços simulados separadamente para a 1ª, 2ª e 3ª hora de aula de cada plano para total clareza da nova regra de descontos individuais.
- **Ajuste de Quebras e Rolagem na Agenda (Desktop)**:
  * **Otimização de Limites de Largura**: Aplicado `max-width: 100%; min-width: 0;` na classe `.horizontal-calendar`. Isso impede que o contêiner de dias expanda a tela de cadastro e a quebre em computadores com janelas estreitas (ex: 770px).
  * **Grid de Horários Fluido**: Reconfigurado `.hours-grid` com `grid-template-columns: repeat(auto-fit, minmax(130px, 1fr))`, dividindo automaticamente os 4 botões em 2 colunas com 2 linhas e impedindo que sejam cortados na direita.
  * **Arrastar para Rolar (Drag-to-Scroll)**: Integrado no script `bindHorizontalScrolls` a captura de movimentos de arrasto do mouse (`mousedown` / `mousemove`), permitindo arrastar o calendário de datas lateralmente no desktop exatamente como em telas de toque.
